...
const profitCards = [
  { icon: iconMarket, title: 'Рынок СНГ свободен' },
  { icon: iconEntry, title: 'Низкий порог входа в бизнес' },
  { icon: iconProfit, title: 'Прибыль в месяц в сети 10 аппаратов' },
  { icon: iconIncome, title: 'Годовая доходность бизнеса' },
];
...
export default function ProfitSection({ onOpenEmailForm }: ProfitSectionProps) {
  ...
  return (
    <section id="profit" ref={ref} className="py-20 lg:py-32 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-display font-bold section-title">Что мы предлагаем вам</h2>
          <p className="text-xl text-muted-foreground mt-6 max-w-2xl mx-auto">
            Начните бизнес с нашей поддержкой, с минимальными инвестициями и высокой доходностью!
          </p>
        </motion.div>

        <motion.div style={{ scale, opacity }} className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {profitCards.map((card, index) => (
            <motion.div
              ...
              whileHover={{ y: -10, scale: 1.05, rotateY: 5, boxShadow: '0 30px 60px hsl(var(--primary) / 0.2)' }}
              className="glass-panel chrome-border p-6 text-center group perspective-1000 preserve-3d"
            >
              <motion.div whileHover={{ rotateY: 360 }} transition={{ duration: 0.8 }} className="w-24 h-24 mx-auto mb-4">
                <img src={card.icon} alt="вендинговые аппарат PARFUMEPOINT" className="w-full h-full object-contain" />
              </motion.div>
